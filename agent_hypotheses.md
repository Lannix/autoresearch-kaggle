...
- [ ] **HYP-12.8: Explicit Periodic Loss with Dynamic Loss Balancing**
  ...
  - *Outcome:* [ ] | *Delta:* [ ]
  - *Notes:* Add a manual periodic value-matching residual inside `pde()` by comparing `(u, v)` at `theta_min` and `theta_max` over a small fixed time grid. Drive it with a callback that updates a scalar periodic weight from logged loss magnitudes, nudging the periodic channel toward the same scale as the mean PDE loss while keeping the current MsFFN, curriculum, and progressive-R3 baseline unchanged.
