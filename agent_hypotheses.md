...
- [x] **HYP-10.4: Transolver-Style Linear Attention (Physics-Attention)**
  ...
  - *Outcome:* [DISCARD] | *Delta:* `+4.888491e-02 val_mse regression`
  - *Notes:* Starting from the kept `HYP-12.4` baseline, inserted an explicit low-rank linear-attention block directly after the encoded Fourier feature bank: `Q`, `K`, and `V` were projected to width `24`, the global physics summary was computed as `K^T V / N`, and the attended response `Q (K^T V)` was fed back into the encoded features through a small residual gain. The run stayed fully stable, avoided the old fused-SDPA crash entirely, preserved the normal curriculum schedule, and still hit the winning step-`5000` R3 refresh on time. Even so, final `val_mse` regressed badly from `3.323696e-02` to `8.212187e-02`, while peak VRAM rose to `2352.4 MB` and parameters rose to `84507`. The most likely failure mode is that this attention block couples every point to a batch-global feature summary, so it can drive the collocation loss very low while still generalizing poorly when the evaluation batches have a different point mix.
...
- [x] **HYP-11.11: Denser Static-and-R3 Collocation Set**
  ...
  - *Outcome:* [DISCARD] | *Delta:* `+7.877870e-03 val_mse regression`
  - *Notes:* Starting from the kept `HYP-12.4` baseline, increased the anchor count from `30000` to `45000` while leaving the winning Gaussian-plus-uniform theta bias, `Beta(1, 3)` time bias, curriculum thresholds, and progressive one-shot R3 logic unchanged. The run stayed stable and the curriculum still opened the full time horizon by step `4000`, but the heavier collocation set slowed Adam enough that training handed off to L-BFGS immediately after stage 4 and the critical step-`5000` R3 refresh never fired. Final `val_mse` regressed from `3.323696e-02` to `4.111483e-02`, peak VRAM rose from `2133.1 MB` to `3120.6 MB`, and total progress fell to `6536` steps, so this first denser-anchor variant loses more from missing R3 than it gains from the extra collocation coverage.
...
- [x] **HYP-13.2: High-Frequency-Capable Complex Representation**
  ...
  - *Outcome:* [DISCARD] | *Delta:* `+1.584958e-02 val_mse regression`
  - *Notes:* Starting from the kept `HYP-12.4` baseline, tested a stabilized high-frequency variant rather than rerunning the earlier full-sine or complex-core failures: a tiny SIREN-style residual branch was added in parallel with the winning MsFFN head, operating directly on normalized `(theta, t)` and scaled by a small trainable gain initialized at `0.05`. This preserved the exact hard-IC ansatz, Gaussian-plus-uniform theta sampler, `Beta(1, 3)` time bias, curriculum, and progressive one-shot R3 refresh. The run stayed fully stable, reached the full domain by step `3000`, and still fired the step-`5000` R3 refresh on time, but final `val_mse` regressed from `3.323696e-02` to `4.908654e-02` with `2271.3 MB` peak VRAM and `8915` total steps. The weaker retained residual scores during R3 (`retained_mean = 1.772e-01`) suggest the sinusoidal correction made the collocation set look easier without improving the true solution, so this stabilized high-frequency residual branch is a discard.

- [x] **HYP-13.3: XPINN-Style Time Slab Decomposition**
  ...
  - *Outcome:* [DISCARD] | *Delta:* `+1.210722e-02 val_mse regression`
  - *Notes:* Starting from the kept `HYP-12.4` baseline, replaced the single global residual head with an XPINN-style two-slab decomposition. Two separate MsFFN-style subnetworks were assigned to early and late time windows, with a hard handoff at `t = t_min + 0.5 * (t_max - t_min)`: the late slab starts from the early slab's predicted interface state and adds its own gated correction, so continuity is built into the ansatz. To keep runtime manageable, each slab head used a slightly smaller `96 x 4` MLP instead of the original `128 x 5`. The run stayed fully stable, preserved the usual curriculum schedule, still reached the full-domain step-`5000` R3 refresh, and even reduced parameter count to `71812` with peak VRAM essentially flat at `2103.7 MB`. But final `val_mse` still regressed from `3.323696e-02` to `4.534418e-02`, so this first slab-decomposition version does not outperform the simpler monolithic baseline.
