...
- [x] **1HR-6.3: True RAR-Style R3 Refactor**
...
- [x] **1HR-6.4: Frequent True RAR Without Global Power Prior**
...
- [x] **1HR-6.5: Partitioned 50k Collocation with Refreshable R3 Pool**
  ...
  - *Outcome:* [DISCARD] | *Delta:* `+6.865810e-03 val_mse regression`
  - *Notes:* Starting from the `02f5f42` direction rather than the kept baseline, this variant kept the global power prior disabled and the true candidate-screened RAR logic active, but increased the anchor budget to `50000` and explicitly split it into `20000` fixed anchors plus a `30000` refreshable R3 pool. The implementation behaved the way we wanted: the static block remained untouched, R3 only scored and updated the refreshable pool, and each `500`-step refresh preserved the strongest refreshable points while replacing only the lowest-error refreshable slice with screened candidates. Even so, the larger anchor budget and constant refresh cycle were still too expensive and too destabilizing for generalization: peak VRAM rose sharply to `3476.4 MB`, total progress fell to `11559` steps, and final `val_mse` regressed from the kept `1HR-0.1` best `3.093994e-02` to `3.780575e-02`. So the pool partition was operationally correct, but it still did not recover the strong generalization of the simpler 1-hour baseline.
