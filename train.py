...
def late_periodicity_losses(device, dtype):
    ...
    late_uv = net.forward_from_normalized(net.normalize_inputs(periodicity_tensors["late"]))
    ref_uv = net.forward_from_normalized(net.normalize_inputs(periodicity_tensors["ref"]))
...
def pde(x, y):
    ...
    return [causal_weight * res_u, causal_weight * res_v, power_stabilization, late_period_u, late_period_v]
...
# Loss weights order corresponds to [pde_u, pde_v, global_power_dt, late_period_u, late_period_v].
loss_weights =[3.0, 3.0, 0.5, 0.5, 0.5]
